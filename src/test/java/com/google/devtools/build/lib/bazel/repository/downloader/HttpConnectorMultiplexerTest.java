...
public class HttpConnectorMultiplexerTest {
  ...
  public void ftpUrl_throwsIae() throws Exception {
    ...
    multiplexer.connect(new URL("ftp://lol.example"), null, ImmutableMap.of(), Optional.absent());
  }
    ...
    Thread task =
        new Thread(
            () -> {
              ...
              try {
                multiplexer.connect(
                    new URL("http://lol.example"), null, ImmutableMap.of(), Optional.absent());
  ...
  public void singleUrl_justCallsConnector() throws Exception {
    assertThat(
            toByteArray(
                multiplexer.connect(URL1, DUMMY_CHECKSUM, ImmutableMap.of(), Optional.absent())))
        .isEqualTo(data1);
